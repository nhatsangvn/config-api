...
class DBInterface(object):
    ...
    def _network_neutron_to_vnc(self, network_q, oper):
        ...
        # CEM-27586: Read mtu from project and assign it to VN if not
        # specified on VN else assign deafult 1500 if project and VN
        # MTU is None
        # CEM-23123:VNC openstack changes for adding MTU attribute to VN
        if 'mtu' in network_q and network_q['mtu'] != 0:
            net_obj.mtu = network_q['mtu']
        elif 'tenant_id' in network_q:
            project_obj = self._get_project_obj(network_q)
            mtu = project_obj.get_mtu()
            if mtu is not None:
                net_obj.mtu = mtu
            else:
                net_obj.mtu = DEFAULT_MTU
