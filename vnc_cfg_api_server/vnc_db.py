...
from cfgm_common.utils import _DEFAULT_ZK_DB_WALK_SYNC_UVE_PATH_PREFIX
from cfgm_common.utils import _DEFAULT_ZK_DB_WALK_SYNC_UVE_COMPLETE_PATH_PREFIX
...
class VncZkClient(object):
    ...
    _RI_ID_ALLOC_PATH = "/id/routing-instances/"
    _RI_MIN_ID = 11
    _RI_MAX_ID = 1 << 24
    ...
    def __init__(self, instance_id, zk_server_ip, host_ip, reset_config, db_prefix,
        ...
        _ri_id_alloc_path = zk_path_pfx + self._RI_ID_ALLOC_PATH
        ...
        self._pi_ae_id_path = zk_path_pfx + self._PI_AE_ID_PATH
        ...
        if reset_config:
            ...
            self._zk_client.delete_node(_ri_id_alloc_path, True)
        ...
        # Initialize the routing instance ID allocator
        self._ri_id_allocator = IndexAllocator(self._zk_client,
                                              _ri_id_alloc_path,
                                              self._RI_MAX_ID)
    ...
    def alloc_ri_id(self, fq_name_str, id=None):
         # If ID provided, it's a notify allocation, just lock allocated ID in
         # memory
         if id is not None:
             if self.get_ri_from_id(id) is not None:
                 self._ri_id_allocator.set_in_use(id - self._RI_MIN_ID)
                 return id
         elif fq_name_str is not None:
             return self._ri_id_allocator.alloc(fq_name_str) + self._RI_MIN_ID

    def free_ri_id(self, id, fq_name_str, notify=False):
         if id is not None and id - self._RI_MIN_ID < self._RI_MAX_ID:
             # If fq_name associated to the allocated ID does not correpond to
             # freed resource fq_name, keep zookeeper lock
             allocated_fq_name_str = self.get_ri_from_id(id)
             if (allocated_fq_name_str is not None and
                     allocated_fq_name_str != fq_name_str):
                 return
             if notify:
                 # If notify, the ZK allocation already removed, just remove
                 # lock in memory
                 self._ri_id_allocator.reset_in_use(id - self._RI_MIN_ID)
             else:
                 self._ri_id_allocator.delete(id - self._RI_MIN_ID)
    ...
    def get_ri_from_id(self, id):
        if id is not None and id - self._RI_MIN_ID < self._RI_MAX_ID:
            return self._ri_id_allocator.read(id - self._RI_MIN_ID)
...
class VncDbClient(object):
    def __init__(self, api_svr_mgr, db_srv_list, rabbit_servers, rabbit_port,
        ...
        self._UVEMAP = {
            ...
            "config_rabbitmq_node" : ("ObjectConfigRabbitmqInfo", True),
        ...
        this_api_server = str(self._api_svr_mgr._args.listen_ip_addr) + \
            str(self._api_svr_mgr.get_worker_id())
        zk_sync_uve_node = self._zk_db._zk_client.read_node(
            _DEFAULT_ZK_DB_WALK_SYNC_UVE_COMPLETE_PATH_PREFIX)
        if zk_sync_uve_node == this_api_server:
            self._zk_db._zk_client.delete_node(
                _DEFAULT_ZK_DB_WALK_SYNC_UVE_COMPLETE_PATH_PREFIX)
    ...
    def db_resync(self):
        ...
        with ZookeeperLock(**zk_dbe_lock):
            try:
                ...
                self.config_log("DBERESYNC: found exception (%s)" %
                                e, level=SandeshLevel.SYS_ERR)
                raise
        if walk_only:
            lockdata = '%s' % self._api_svr_mgr._args.listen_ip_addr + \
                '_' + '%s' % self._api_svr_mgr.get_worker_id()
            zk_sync_uve_lock = {'zookeeper_client': self._zk_db._zk_client,
                                'path': _DEFAULT_ZK_DB_WALK_SYNC_UVE_PATH_PREFIX,
                                'name': lockdata, 'timeout': _DEFAULT_ZK_LOCK_TIMEOUT / 2}

            self.config_log("DBERESYNC: running walk",
                            level=SandeshLevel.SYS_INFO)

            sync_uves = False
            with ZookeeperLock(**zk_sync_uve_lock):
                try:
                    zk_sync_uve_node = self._zk_db._zk_client.read_node(
                        _DEFAULT_ZK_DB_WALK_SYNC_UVE_COMPLETE_PATH_PREFIX)
                    self.config_log("zk_sync_uve_node data (%s)" %
                                    zk_sync_uve_node, level=SandeshLevel.SYS_INFO)
                    if not zk_sync_uve_node:
                        data_for_zk_lock_uve = str(self._api_svr_mgr._args.listen_ip_addr) +\
                            str(self._api_svr_mgr.get_worker_id())
                        self._object_db.walk(self.synchronize_uves)
                        self._zk_db._zk_client.create_node(
                            _DEFAULT_ZK_DB_WALK_SYNC_UVE_COMPLETE_PATH_PREFIX, data_for_zk_lock_uve)

                    else:
                        sync_uves = True
                except Exception as e:
                    self.config_log("ERROR synchronizing UVEs: %s" %
                                    e, level=SandeshLevel.SYS_ERR)
                    raise
            if sync_uves:
    ...
    def synchronize_uves(self, obj_type, obj_uuids):
        obj_class = cfgm_common.utils.obj_type_to_vnc_class(obj_type, __name__)
        obj_fields = list(obj_class.prop_fields) + list(obj_class.ref_fields)
        (ok, obj_dicts) = self._object_db.object_read(
            obj_type, obj_uuids, field_names=obj_fields)
        ...
        uve_trace_list = []
        for obj_dict in obj_dicts:
            uve_trace_list.append(
                ("RESYNC", obj_type, obj_dict['uuid'], obj_dict))
        # Send UVEs resync with a pool of workers
        uve_workers = gevent.pool.Group()
    ...
    def _dbe_resync_worker(self, obj_type, obj_dict, **kwargs):
            ...
            try:
                ...
                if obj_type == 'virtual_network':
                    ...
                    if 'routing_instance_id' not in obj_dict:
                        ri_id = self._zk_db.alloc_ri_id(
                                ':'.join(obj_dict['fq_name']))
                        obj_dict['routing_instance_id'] = ri_id
                        self._object_db.object_update('routing_instance',
                                                      obj_uuid, obj_dict)
