...
class VncApiServer(object):
    ...
    def alloc_ri_id(self, fq_name_str):
        return self._db_conn._zk_db.alloc_ri_id(fq_name_str)
    ...
    def create_default_children(self, object_type, parent_obj):
        ...
        for child_type in child_types:
            ...
            # For Routing Instances, allocate an ID
            if child_obj_type == 'routing_instance':
                child_dict['routing_instance_id'] = self.alloc_ri_id(
                    child_obj.get_fq_name_str())
    ...
    def route(self, uri, method, handler):
        ...
        print("vng-route %s - %s - %s" % (str(uri), str(method), str(handler)))
   ...
   # end
    ...
    def fq_name_to_id_http_post(self):
        ...
        if request.json is not None:
            self.config_log("vng-not-none " + str(request), level=SandeshLevel.SYS_INFO)
            type =request.json.get('type')
            # if type == "project":
            #     self.config_log("fq_name_to_id_http_post print parsed: " + str(request.json),level=SandeshLevel.SYS_DEBUG)
        else:
            #self.config_log("fq_name_to_id_http_post print body: " + str(request.body.getvalue()),level=SandeshLevel.SYS_DEBUG)
            #request_data = {key: str(value) for key, value in request.__dict__.items()}
            self.config_log("vng " + str(request), level=SandeshLevel.SYS_INFO)

            for key, value in request.headers.items():
                self.config_log("fq_name_to_id_http_post print headers: " + str(key) + ":" + str(value),level=SandeshLevel.SYS_INFO)
            self.config_log("fq_name_to_id_http_post print environ: " + str(request.environ),level=SandeshLevel.SYS_INFO)
            raise cfgm_common.exceptions.HttpError(500,"fail to parse")
        ### VNG: add to debug ###
        #type = get_request().json.get('type')
    ...
    def create_singleton_entry(self, singleton_obj, user_visible=True):
        ...
        try:
            ...
            if obj_type == 'routing_instance':
                ri_id = self.alloc_ri_id(s_obj.get_fq_name_str())
