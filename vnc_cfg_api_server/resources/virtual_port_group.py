...
from vnc_cfg_api_server.resources.virtual_machine_interface \
    import VirtualMachineInterfaceServer
...
class VirtualPortGroupServer(ResourceMixin, VirtualPortGroup):
    ...
    @classmethod
    def _alloc_ae_id(cls, prouter_name, pi_uuid, vpg_name, vpg_uuid,
                     ae_num=None):
            ...
            while True:
                try:
                    ...
                    if ae_id is not None:
                        try:
                            # CEM-28177: create zk_pi_ae_node to track how
                            # many PIs are associated with this ae_id
                            pi_ae_path = os.path.join(
                                cls.vnc_zk_client._pi_ae_id_path,
                                '%s' % prouter_name,
                                '%s' % ae_id,
                                '%s' % pi_uuid)
                            msg = ("Creating Znode for prouter(%s):AE-ID(%s):"
                                   "PI(%s):vpg(%s)" % (prouter_name, ae_id,
                                                       pi_uuid, vpg_name))
                            cls.db_conn.config_log(
                                msg, level=SandeshLevel.SYS_DEBUG)
                    ...
                    def undo_alloc_ae_id():
                        pi_ae_prefix_path = os.path.join(
                            cls.vnc_zk_client._pi_ae_id_path,
                            '%s' % prouter_name,
                            '%s' % ae_id)
                        pi_zk_nodes = \
                            cls.vnc_zk_client._zk_client.get_children(
                                pi_ae_prefix_path)
                        for pi_zk_node in pi_zk_nodes:
                            if len(pi_zk_nodes) > 1:
                                msg = ("Skipping deallocation as AE-ID(%s) is "
                                       "associated at pi_zk_node(%s)" %
                                       (ae_id, pi_zk_node))
                                cls.db_conn.config_log(
                                    msg, level=SandeshLevel.SYS_DEBUG)
                            if len(pi_zk_nodes) == 1 and \
                                    (pi_zk_node == pi_uuid):
                                ok, result = cls._dealloc_ae_id(
                                    prouter_name, ae_id, vpg_name,
                                    vpg_uuid)
                                return ok, result
                            else:
                                msg = ("Skipping deallocation as current "
                                       "pi_uuid(%s) is not associated with "
                                       "AE-ID(%s) " % (pi_uuid, ae_id))
                                cls.db_conn.config_log(
                                    msg, level=SandeshLevel.SYS_DEBUG)
    ...
    def _dealloc_ae_id(cls, prouter_name, ae_id, vpg_name, vpg_uuid):
        ...
        if pi_ae is not None and pi_ae == ae_id:
            ae_id = pi_ae
            # CEM-28177: delete all zk-nodes created for the ae_id with
            # same PI/PR
            pi_ae_prefix_path = os.path.join(
                cls.vnc_zk_client._pi_ae_id_path,
                '%s' % prouter_name,
                '%s' % ae_id)
            cls.vnc_zk_client._zk_client.delete_node(pi_ae_prefix_path, True)
    ...
    def _process_ae_id(cls, db_obj_dict, vpg_name, obj_dict=None):
        ...
        for pi_uuid in create_pi_uuids:
            ...
            if pi_ae is None:
                ...
                ok, result = cls._alloc_ae_id(
                    pi_pr, pi_uuid, vpg_name, vpg_uuid, ae_num)
                if not ok:
                    return ok, result
            ...
            if (len(db_pi_dict.keys()) == 1 and len(create_pi_uuids) == 1):
                ...
                if (list(db_pi_dict.values())[0] !=
                    ...
                    ok, result = cls._alloc_ae_id(db_pr, pi_uuid, vpg_name,
                                                  vpg_uuid)
                    if not ok:
                        return ok, result
        ...
        for pi_uuid in delete_pi_uuids:
            ...
            pi_ae = db_pr_dict.get(pi_pr)
            # CEM-28177: delete all PI/PR ae_nodes
            ...
            if pi_pr is not None:
                try:
                    pr_name = cls.db_conn.uuid_to_fq_name(pi_pr)
                    pi_ae_path = os.path.join(
                        cls.vnc_zk_client._pi_ae_id_path,
                        '%s' % pr_name[-1],
                        '%s' % ae_id,
                    ...
                    cls.vnc_zk_client._zk_client.delete_node(
                        pi_ae_path, True)
                    msg = ("Deleting all AE-PI znodes created for current "
                           "PR(%s):PI(%s):AE-ID(%s)" %
                           (pr_name[-1], pi_uuid, ae_id))
                    cls.db_conn.config_log(msg, level=SandeshLevel.SYS_DEBUG)
                except NoIdError:
                    msg = "PI-AE znode path(%s) doesn't exists...Skipping"
                    cls.db_conn.config_log(msg, level=SandeshLevel.SYS_DEBUG)
                    pass
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        if kwargs['ref_update']:
            if kwargs['ref_update']['operation'] == 'DELETE' and \
                kwargs['ref_update']['ref_obj_type'] == \
                    'virtual_machine_interface':
                vmi_uuid = kwargs['ref_update']['ref_uuid']
                ok, vmi_data = db_conn.dbe_read(
                    obj_type='virtual_machine_interface',
                    obj_id=vmi_uuid
                )
                if not ok:
                    return ok, (400, vmi_data)
                if vmi_data.get('virtual_port_group_refs'):
                    if vmi_data['virtual_port_group_refs'][0]['uuid'] == id:
                        msg = "Ref update for vmi (%s) to delete vpg"\
                              "refs (%s)" % (vmi_uuid, id)
                        db_conn.config_log(msg, level=SandeshLevel.SYS_INFO)
                        (ok, result) = db_conn.ref_update(
                            ...
                            {"attr": None},
                            "DELETE",
                            vmi_data.get("id_perms")
                        )

                        def undo_vpg_ref_from_vmi():
                            (ok, result) = db_conn.ref_update(
                                "virtual_machine_interface",
                                vmi_uuid,
                                "virtual_port_group",
                        ...
                        get_context().push_undo(undo_vpg_ref_from_vmi)
                        ok, vpg_dict = db_conn.dbe_read(
                            obj_type='virtual_port_group',
                            obj_id=obj_dict['uuid'])
                        if not ok:
                            return ok, (400, vpg_dict)
                        ok = VirtualMachineInterfaceServer \
                            ._delete_validations_znodes(
                                vpg_dict, vmi_data, db_conn, id)
                        if not ok:
                            msg = "Znode deletion failed for vmi (%s)"\
                                  "with vpg (%s)"\
                                  % (vmi_uuid, id)
                            return False, (400, msg)
        return True, ret_val
    ...
    def post_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        for pi_ref in pi_refs:
            ...
            if ae_pi is not None:
                ...
                if ae_pi == ae_id:
                    continue
            ok, result = cls._alloc_ae_id(prouter_name, pi_uuid, vpg_name,
                                          vpg_uuid)
            if not ok:
                return ok, result
            attr_dict, _alloc_dict = result
