...
class BgpRouterServer(ResourceMixin, BgpRouter):
    ...
    def post_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        router_type = obj_dict['bgp_router_parameters']['router_type']
        if ('global_system_config_back_refs' not in obj_dict and
                (router_type == 'control-node' or
                 router_type == 'external-control-node')):
            ok, gsc = cls.server.get_resource_class(
                'global_system_config')._get_global_system_config()
            if ok:
