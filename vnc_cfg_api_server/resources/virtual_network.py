...
from cfgm_common import DEFAULT_MTU
...
class VirtualNetworkServer(ResourceMixin, VirtualNetwork):
    ...
    def pre_dbe_create(cls, tenant_name, obj_dict, db_conn):
        ...
        # Allocate MTU value from project/tenant, if not defined explicitly
        if 'mtu' not in obj_dict or obj_dict['mtu'] == 0:
            ok, proj_dict = db_conn.dbe_read('project',
                                             obj_dict['parent_uuid'])
            if 'mtu' in proj_dict and proj_dict['mtu'] is not None:
                obj_dict['mtu'] = proj_dict['mtu']
            else:
                obj_dict['mtu'] = DEFAULT_MTU
    ...
    def pre_dbe_update(cls, id, fq_name, obj_dict, db_conn, **kwargs):
        ...
        # Allocate MTU value from project/tenant, if not defined explicitly
        ok, vn_obj = cls.dbe_read(db_conn, 'virtual_network', id)
        if 'mtu' not in obj_dict and 'parent_uuid' in obj_dict\
                and 'mtu' in vn_obj:
            ok, proj_dict = db_conn.dbe_read('project',
                                             obj_dict['parent_uuid'])
            if 'mtu' in proj_dict and proj_dict['mtu'] is not None:
