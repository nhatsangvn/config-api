...
class TestVirtualPortGroup(TestVirtualPortGroupBase):
    ...
    def test_detach_vmi_from_vpg_enterprise(self):
        """Verify vmi is detached from enterprise VPG."""
        proj_obj, fabric_obj, pr_obj = self._create_prerequisites()
        ...
        # Create KV_Pairs for this VMI
        kv_pairs_1 = self._create_kv_pairs(pi_fq_name,
                                           fabric_name,
                                           vpg_name)
        vmi_obj_1.set_virtual_machine_interface_bindings(kv_pairs_1)
        ...
        znode_vlan_1_id = mock_zk._zk_client.read_node(
            vlan_validation_node1)
        tagged_validation_path_1 = os.path.join(
            _DEFAULT_ZK_FABRIC_ENTERPRISE_PATH_PREFIX,
            'virtual-port-group:%s' % vpg_uuid,
            'vlan:%s' % znode_vlan_1_id)
        ...
        # Create KV_Pairs for this VMI with an untagged VLAN
        # If tor_port_vlan_id is set, then it signifies a untagged VMI
        kv_pairs_2 = self._create_kv_pairs(pi_fq_name,
                                           fabric_name,
                                           vpg_name,
                                           tor_port_vlan_id=vlan_2)

        vmi_obj_2.set_virtual_machine_interface_bindings(kv_pairs_2)
        vmi_obj_2.set_virtual_machine_interface_properties(
            VirtualMachineInterfacePropertiesType(
                sub_interface_vlan_tag=vlan_2))
        ...
        # Verify VMI refs to VPG
        vpg_before_update = self.api.virtual_port_group_read(id=vpg_uuid)
        vmi_refs = vpg_before_update.__dict__['virtual_machine_interface_refs']
        assert len(vmi_refs) == 2, "VPG is not having two VMI refs"
        # ref-update to delete VMI1 refs to VPG
        self.api.ref_update("virtual-port-group", vpg_obj.uuid,
                            "virtual-machine-interface", vmi_uuid_1, None,
                            "DELETE", None)

        # Read VPG and VMI1 after deleting VMI1 refs from VPG
        self.api.virtual_port_group_update(vpg_obj)
        vpg_update_1 = self.api.virtual_port_group_read(id=vpg_obj.uuid)
        updated_vmi_refs = \
            vpg_update_1.__dict__['virtual_machine_interface_refs'
                                  ][0]['uuid']
        vmi_ref_1 = vpg_update_1.__dict__['virtual_machine_interface_refs']
        assert len(vmi_ref_1) == 1 and updated_vmi_refs == vmi_uuid_2, \
            "VMI1 ref is not deleted, VPG still have two VMI refs"
        # Verify if Znode is present
        znode1 = mock_zk._zk_client.read_node(vlan_validation_node1)
        tagged_znode = mock_zk._zk_client.read_node(tagged_validation_path_1)

        assert (tagged_znode is None) or (znode1 is None), \
            "Znode for VMI_1 (%s) exists" % vmi_uuid_1
        # ref-update to delete VMI2 refs from VPG
        self.api.ref_update("virtual-port-group", vpg_obj.uuid,
                            "virtual-machine-interface", vmi_uuid_2, None,
                            "DELETE", None)

        # Read VPG, VMI1 and VMI2 after deleting both VMI refs from VPG
        self.api.virtual_port_group_update(vpg_obj)
        vpg_obj_update = self.api.virtual_port_group_read(id=vpg_obj.uuid)
        vmi1_obj_update = \
            self.api.virtual_machine_interface_read(id=vmi_uuid_1)
        vmi2_obj_update = \
            self.api.virtual_machine_interface_read(id=vmi_uuid_2)

        # Verify VMI1 and VMI2 after deleting VMI refs.
        assert vmi1_obj_update.__dict__ and vmi2_obj_update.__dict__, \
            "VMI1 and VMI2 doesn't exists."

        # Verify if Znode is present
        znode_untagged = mock_zk._zk_client.read_node(untagged_validation_path)
        assert znode_untagged is None, \
            "Znode for VMI_2 (%s) exists" % vmi_uuid_2

        # Verify if VPG has VMI refs.
        if 'virtual_machine_interface_refs' \
                not in vpg_obj_update.__dict__.keys():
            logger.info("VPG doesn't have VMI refs")
        else:
            self.fail("After ref update VMI refs still exist.")

    # End of Test case

    def test_detach_sp_vpg_znode_validations(self):
        ...
        vpg_before_update = self.api.virtual_port_group_read(id=vpg_uuid)
        vmi_refs = vpg_before_update.__dict__['virtual_machine_interface_refs']
        assert len(vmi_refs) == 2, \
            "VPG is not having two VMI refs"
        ...
        vmi_ref_lst = vpg_update_1.__dict__['virtual_machine_interface_refs']
        assert len(vmi_ref_lst) == 1 and \
            updated_vmi_refs == vmi_uuid_2, \
            "VMI1 ref is not deleted and VPG having two VMI refs."
        # Verify if Znode is added back
        tagged_znode = mock_zk._zk_client.read_node(tagged_validation_path)
        assert tagged_znode is None, \
            "Znode for VMI_1 (%s) exists" % vmi_uuid_1
