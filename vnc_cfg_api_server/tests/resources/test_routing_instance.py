...
class TestRoutingInstance(test_case.ApiServerTestCase):
    ...
    def test_allocate_ri_id(self):
        ...
        ri_fq_name = vn.fq_name + [ri.name]
        ri_obj = self.api.routing_instance_read(ri_fq_name)
        ri_id = ri_obj.routing_instance_id

        self.assertEqual(ri_obj.get_fq_name_str(),
                         mock_zk.get_ri_from_id(ri_id))
        self.assertGreaterEqual(ri_id, mock_zk._RI_MIN_ID)

    def test_deallocate_ri_id(self):
        mock_zk = self._api_server._db_conn._zk_db
        ...
        self.api.routing_instance_delete(ri_fq_name)

        self.assertNotEqual(mock_zk.get_ri_from_id(ri_id),
                            ri_obj.get_fq_name_str())

    def test_not_deallocate_ri_id_if_fq_name_does_not_correspond(self):
        mock_zk = self._api_server._db_conn._zk_db
        vn = VirtualNetwork('%s-vn' % self.id())
        self.api.virtual_network_create(vn)
        ...
        mock_zk._ri_id_allocator.delete(ri_id - mock_zk._RI_MIN_ID)
        mock_zk._ri_id_allocator.reserve(ri_id - mock_zk._RI_MIN_ID,
                                         fake_fq_name)
        self.api.routing_instance_delete(ri_fq_name)

        self.assertIsNotNone(mock_zk.get_ri_from_id(ri_id))
        self.assertEqual(fake_fq_name, mock_zk.get_ri_from_id(ri_id))

    def test_cannot_set_ri_id(self):
        vn = VirtualNetwork('%s-vn' % self.id())
        self.api.virtual_network_create(vn)

        ri = RoutingInstance('ri-%s' % self.id(), parent_obj=vn)
        ri.set_routing_instance_is_default(False)
        ri.set_routing_instance_id(42)

        with ExpectedException(PermissionDenied):
            self.api.routing_instance_create(ri)

    def test_cannot_update_ri_id(self):
        vn = VirtualNetwork('%s-vn' % self.id())
        self.api.virtual_network_create(vn)
        ...
        ri_obj.set_routing_instance_id(42)
        with ExpectedException(PermissionDenied):
            self.api.routing_instance_update(ri_obj)
