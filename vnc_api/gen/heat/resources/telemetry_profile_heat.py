...
class ContrailTelemetryProfile(contrail.ContrailResource):
    ...
    properties_schema = {
        ...
        GRPC_PROFILE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('GRPC_PROFILE_REFS.'),
            update_allowed=True,
            required=False,
        ...
        SNMP_PROFILE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('SNMP_PROFILE_REFS.'),
            update_allowed=True,
            required=False,
        ),
        NETCONF_PROFILE_REFS: properties.Schema(
            properties.Schema.LIST,
            _('NETCONF_PROFILE_REFS.'),
            update_allowed=True,
            required=False,
        ),
    ...
    attributes_schema = {
        ...
        GRPC_PROFILE_REFS: attributes.Schema(
            _('GRPC_PROFILE_REFS.'),
        ),
        SFLOW_PROFILE_REFS: attributes.Schema(
            _('SFLOW_PROFILE_REFS.'),
        ),
        SNMP_PROFILE_REFS: attributes.Schema(
            _('SNMP_PROFILE_REFS.'),
        ),
        NETCONF_PROFILE_REFS: attributes.Schema(
            _('NETCONF_PROFILE_REFS.'),
        ),
    ...
    def handle_create(self):
        ...
        # reference to grpc_profile_refs
        if self.properties.get(self.GRPC_PROFILE_REFS):
            for index_0 in range(len(self.properties.get(self.GRPC_PROFILE_REFS))):
                try:
                    ref_obj = self.vnc_lib().grpc_profile_read(
                        id=self.properties.get(self.GRPC_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().grpc_profile_read(
                        fq_name_str=self.properties.get(self.GRPC_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_grpc_profile(ref_obj)
        ...
        # reference to snmp_profile_refs
        if self.properties.get(self.SNMP_PROFILE_REFS):
            for index_0 in range(len(self.properties.get(self.SNMP_PROFILE_REFS))):
                try:
                    ref_obj = self.vnc_lib().snmp_profile_read(
                        id=self.properties.get(self.SNMP_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().snmp_profile_read(
                        fq_name_str=self.properties.get(self.SNMP_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_snmp_profile(ref_obj)

        # reference to netconf_profile_refs
        if self.properties.get(self.NETCONF_PROFILE_REFS):
            for index_0 in range(len(self.properties.get(self.NETCONF_PROFILE_REFS))):
                try:
                    ref_obj = self.vnc_lib().netconf_profile_read(
                        id=self.properties.get(self.NETCONF_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().netconf_profile_read(
                        fq_name_str=self.properties.get(self.NETCONF_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                obj_0.add_netconf_profile(ref_obj)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        # reference to grpc_profile_refs
        ref_obj_list = []
        if self.GRPC_PROFILE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.GRPC_PROFILE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().grpc_profile_read(
                        id=prop_diff.get(self.GRPC_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().grpc_profile_read(
                        fq_name_str=prop_diff.get(self.GRPC_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_grpc_profile_list(ref_obj_list)
            # End: reference to grpc_profile_refs
        ...
        # reference to snmp_profile_refs
        ref_obj_list = []
        if self.SNMP_PROFILE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.SNMP_PROFILE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().snmp_profile_read(
                        id=prop_diff.get(self.SNMP_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().snmp_profile_read(
                        fq_name_str=prop_diff.get(self.SNMP_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_snmp_profile_list(ref_obj_list)
            # End: reference to snmp_profile_refs

        # reference to netconf_profile_refs
        ref_obj_list = []
        if self.NETCONF_PROFILE_REFS in prop_diff:
            for index_0 in range(len(prop_diff.get(self.NETCONF_PROFILE_REFS) or [])):
                try:
                    ref_obj = self.vnc_lib().netconf_profile_read(
                        id=prop_diff.get(self.NETCONF_PROFILE_REFS)[index_0]
                    )
                except vnc_api.NoIdError:
                    ref_obj = self.vnc_lib().netconf_profile_read(
                        fq_name_str=prop_diff.get(self.NETCONF_PROFILE_REFS)[index_0]
                    )
                except Exception as e:
                    raise Exception(_('%s') % str(e))
                ref_obj_list.append({'to':ref_obj.fq_name})

            obj_0.set_netconf_profile_list(ref_obj_list)
            # End: reference to netconf_profile_refs
